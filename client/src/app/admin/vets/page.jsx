...
const SPECIALIZATIONS = [
  "General Practice", "Canine", "Feline", "Exotic Pets", "Bird", "Reptile",
  "Livestock", "Large Animals", "Emergency Care", "Surgery", "Dermatology",
  "Cardiology", "Oncology", "Dental Care", "Behavioral", "Nutrition"
];

const CONSULTATION_MODES = ["video", "audio", "chat"];
...
export default function AdminVetsPage() {
  ...
  const fetchVets = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/vet/admin");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(editingVet ? "Vet updated successfully!" : "Vet added successfully!");
        setShowAddForm(false);
        setEditingVet(null);
        resetForm();
        fetchVets();
      } else {
        toast.error(data.message || "Failed to save vet");
      }
    } catch (error) {
  ...
  const handleEdit = (vet) => {
    ...
    setFormData({
      ...
      consultationModes: vet.consultationModes || ["video", "chat"],
      ...
      isActive: vet.isActive ?? true,
      approvalStatus: vet.approvalStatus || "approved"
    });
    setShowAddForm(true);
  };
  ...
  const handleDelete = async (vetId) => {
    ...
    try {
      const response = await fetch(`/api/vet/admin/${vetId}`, {
        method: "DELETE",
        credentials: "include"
      });
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Failed to delete vet");
      }
    } catch (error) {
  ...
  const filteredVets = vets.filter(vet => {
    const matchesSearch = !searchQuery || 
      ...
      vet.licenseNumber.toLowerCase().includes(searchQuery.toLowerCase());
    
    const matchesStatus = filterStatus === "all" || vet.approvalStatus === filterStatus;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-secondary via-white to-secondary/30 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-primary font-semibold">Loading veterinarians...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold titlefont">Veterinarian Management</h1>
                <p className="text-white/80 text-sm mt-1">Manage and add veterinary consultants</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {showAddForm ? "Cancel" : "Add New Vet"}
              </Button>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-primary mb-2">
                        {editingVet ? "New Password (leave blank to keep current)" : "Password *"}
                      </label>
                      <Input
                        type="password"
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-primary mb-2">License Number *</label>
                      <Input
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-primary mb-2">Specializations *</label>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-primary mb-2">Consultation Fee *</label>
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-primary mb-2">Consultation Modes *</label>
                        ...
                        {CONSULTATION_MODES.map(mode => (
                          <label key={mode} className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              className="w-4 h-4 text-primary border-accent/30 rounded"
                            />
                            <span className="text-sm text-primary capitalize">{mode}</span>
                          </label>
                        ))}
                      </div>
                    </div>
          ...
          {filteredVets.length === 0 ? (
            <Card className="bg-white/95 backdrop-blur-md border-2 border-accent/20 shadow-lg">
              <CardContent className="p-16 text-center">
                ...
                <h3 className="text-2xl font-bold text-primary mb-2">No Veterinarians Found</h3>
                <p className="text-secondary mb-6">Get started by adding your first veterinarian</p>
                <Button
            ...
            filteredVets.map((vet) => (
              <Card key={vet._id} className="bg-white/95 backdrop-blur-md border-2 border-accent/20 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="text-2xl font-bold text-primary">{vet.name}</h3>
                            <Badge className={
                              vet.approvalStatus === "approved" ? "bg-green-100 text-green-800" :
                              vet.approvalStatus === "pending" ? "bg-yellow-100 text-yellow-800" :
                              vet.approvalStatus === "rejected" ? "bg-red-100 text-red-800" :
                          ...
                          <div className="grid md:grid-cols-2 gap-4 text-sm text-secondary mb-4">
                            ...
                            {vet.location?.city && (
                              <div className="flex items-center gap-2">
                                ...
                                <span>{vet.location.city}, {vet.location.state}</span>
                              </div>
                            )}
                            <div className="flex items-center gap-2">
                              <DollarSign className="w-4 h-4 text-accent" />
                              <span>{vet.currency} {vet.consultationFee}</span>
                            </div>
                          </div>
                          ...
                          {vet.bio && (
                            <p className="text-secondary text-sm mb-4 line-clamp-2">{vet.bio}</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                      ...
                      <select
                        value={vet.approvalStatus}
                        onChange={(e) => handleStatusChange(vet._id, e.target.value)}
